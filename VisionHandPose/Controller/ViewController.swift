...
class ViewController: UIViewController, HandSwiperDelegate{
    ...
    //MARK: - Properties
    var modelData = [
        DataModel(bgColor: .systemYellow, image: UIImage(named: Constant.ImageName.image2)!),
        DataModel(bgColor: .systemBlue, image: UIImage(named: Constant.ImageName.image3)!),
        DataModel(bgColor: .systemYellow, image: UIImage(named: Constant.ImageName.image4)!),
        DataModel(bgColor: .systemBlue, image: UIImage(named: Constant.ImageName.image5)!),
        DataModel(bgColor: .systemYellow, image: UIImage(named: Constant.ImageName.image6)!),
        DataModel(bgColor: .systemBlue, image: UIImage(named: Constant.ImageName.image7)!),
        DataModel(bgColor: .systemYellow, image: UIImage(named: Constant.ImageName.image8)!),
        DataModel(bgColor: .systemBlue, image: UIImage(named: Constant.ImageName.image9)!),
        DataModel(bgColor: .systemYellow, image: UIImage(named: Constant.ImageName.image1)!),
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        title = Constant.useThumb
        navigationController?.navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.red]

        stackContainer.dataSource = self
    }
    
    private let videoDataOutputQueue = DispatchQueue(label: Constant.cameraFeedData, qos: .userInteractive)
    ...
    func addButtons()
    {
        ...
        leftButton.setImage(UIImage(named: Constant.nope), for: .normal)
        
        leftButton.addTarget(self, action: #selector(onButtonPress(sender:)), for: .touchUpInside)
        leftButton.tag = 0
        
        rightButton = UIButton(type: .custom)
        rightButton.setImage(UIImage(named: Constant.like), for: .normal)
    ...
    func configureNavigationBarButtonItem() {
        let buttonImage = UIImage(named: Constant.undo)?.withRenderingMode(.alwaysOriginal)

            navigationItem.rightBarButtonItem = UIBarButtonItem(image: buttonImage, style: .plain, target: self, action: #selector(resetTapped))
    ...
    func setupAVSession() throws {
        ...
        guard let videoDevice = AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: .front) else {
            throw AppError.captureSessionSetup(reason: Constant.noFrontCamera)
        }
        
        guard let deviceInput = try? AVCaptureDeviceInput(device: videoDevice) else {
            throw AppError.captureSessionSetup(reason: Constant.notCreateVideo)
        }
        ...
        guard session.canAddInput(deviceInput) else {
            throw AppError.captureSessionSetup(reason: Constant.notAddVideoDevice)
        }
        ...
        if session.canAddOutput(dataOutput) {
            ...
            throw AppError.captureSessionSetup(reason: Constant.notAddVideoData)
        }
